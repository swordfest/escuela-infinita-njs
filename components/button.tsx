...
export default function Button(props: any) {
    ...
    if (props.type === 'menu') {
        return (
            <div ref={ref} onClick={()=> setOpen(!open)} title="Descargar Capitulos" className={("relative w-full xl:w-auto h-14 bg-[#98CCA5] hover:bg-[#a5dfb4] select-none flex items-center justify-between xl:justify-center opacity-0 transition-all duration-200 -translate-x-4  cursor-pointer") + (' delay-' + props.delay) + (props.isVisible && (' translate-x-0 opacity-100 delay-' + props.delay))} >
                ...
                <ul className={("menu absolute w-full h-auto top-14 bg-[#174563] text-[#0d2636] font-semibold flex flex-col items-center justify-start transition-transform  ") + (open ? '' : 'hidden')}>
    ...
    if (props.type === 'slider') {
        return (
            <button title="slide-button" name="Descarga" className={(" absolute z-[1] w-10 h-10 bg-[#174563] border-red-100 hover:scale-105 select-none flex items-center justify-center transition-all hover:bg-[#143c57] active:bg-[#194c6e] ") + (props.side === 'left' ? '-left-12 top-[40%] ' : '') + (props.side === 'right' ? '-right-12 top-[40%] ' : '')} >
