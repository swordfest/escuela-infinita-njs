...
export async function getStaticProps() {
	...
	const tags = await fetch(`https://apiei.aprendiendo.cu/wp-json/wp/v2/tags`);
	...
	return (
		<>
			...
			<main className="container h-auto mx-auto grid grid-cols-12 auto-rows-auto gap-[30px] my-10 ">
					...
					{hasMounted &&
						props.result.map((c: any) => (
							<Recurso
								...
								doc={
									props.media.filter((m: any) => {
										return m.id === c.acf.documento_recurso;
									})[0]?.source_url
								}
							/>
						))}
				...
				<div className="blog-sidebar col-span-12 xl:col-span-4 px-4 lg:px-0 h-auto lg:h-screen flex flex-col gap-8">
					<div className="flex flex-col gap-4">
						<span className="text-xl font-semibold">Recursos recientes</span>
						{props.result.slice(0, 4).map((d: any) => (
							<div
								key={d.id}
								className="w-full h-full flex flex-col gap-1 pl-2 ">
